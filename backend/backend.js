...
app.post('/users', async (req, res) => {
  try {
    const { firstName, lastName, email, password: plainTextPassword } =
      req.body;
    const salt = 10;
  ...
  try {
    ...
    jwt.verify(
      ...
      async (err, user) => {
        ...
        const contacts = await Contact.find({ user: user.id, pin: true }).sort({
          firstName: 'asc',
          lastName: 'asc',
        });
        res.json(contacts);
...
app.put('/group/:id/remove', async (req, res) => {
  try {
    ...
    if (!group.contacts.includes(contactId)) {
      return res.status(400).json({ error: 'Contact is not in the group' });
    }
